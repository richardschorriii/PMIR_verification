#!/usr/bin/env python3
"""
PASS41 - PMIR Verification Test
Extracted from ChatGPT transcript (line 88869)
Length: 11419 characters
"""
...
def main():
    ...
    top_robust = BR2[BR2["alpha_break"].isna()].head(20)
    top_fragile = BR2[BR2["alpha_break"].notna()].head(20)

    out_txt.write_text(
        "PASS41 — eigen-competition map (directional mixing robustness)\n"
        f"input: {args.in_csv}\n"
        f"topo_ref: {args.topo_ref}  topo_alt: {args.topo_alt}\n"
        f"dir_base: {args.dir_base}  competitors: {competitors}\n"
        f"alpha_grid: {args.alpha_grid}\n"
        f"beta_tol: {args.beta_tol}  cv_tol: {args.cv_tol}  growth_tol: {args.growth_tol}\n"
        f"boot_reps: {args.boot_reps} seed: {args.seed}\n\n"
        "Most ROBUST (no break up to alpha=1):\n"
        + (top_robust.to_string(index=False) if len(top_robust) else "NONE") + "\n\n"
        "Most FRAGILE (earliest alpha_break):\n"
        + (top_fragile.to_string(index=False) if len(top_fragile) else "NONE") + "\n",
        encoding="utf-8"
    )

    print("[PASS41] wrote", str(out_sw))
    print("[PASS41] wrote", str(out_br))
    print("[PASS41] wrote", str(out_txt))


if __name__ == "__main__":
    main()
Run commands (PowerShell)
$P33SUM = "analysis_transfer\PMIR_TIME_LOCK_MASTER\runs_eps_sweep\probe_invariance\pass33_probe_invariance_auc_FULL\pass33_summary_by_dir_probe_topoN_eps.csv"
$O41    = "analysis_transfer\PMIR_TIME_LOCK_MASTER\runs_eps_sweep\probe_invariance\pass41_eigen_competition"

python .\pass41_eigen_competition_map.py `
  --in_csv  $P33SUM `
  --outdir  $O41 `
  --topo_ref rr `
  --topo_alt grid2d_periodic `
  --dir_base fiedler `
  --dir_competitors smooth,random `
  --alpha_grid 0:1:0.05 `
  --boot_reps 2000 `
  --seed 1337
Outputs: