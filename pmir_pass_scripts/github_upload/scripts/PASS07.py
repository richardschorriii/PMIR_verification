#!/usr/bin/env python3
"""
PASS7 - PMIR Verification Test
Extracted from ChatGPT transcript (line 39759)
Length: 11228 characters
"""
...
def main():
    ...
    for g in graph_seeds:
        for p in pmir_seeds:
            ...
            # stage baseline once
            base_stage = stage_root / f"base_topo={args.topology}_N{args.N}_g{g}_p{p}"
            stage_curve_for_tauhalf(base_curve, base_stage, args.N, "curve")

            # stage +eps
            plus_stage = stage_root / f"plus_topo={args.topology}_N{args.N}_g{g}_p{p}_eps{abs(args.eps):.5f}"
            stage_curve_for_tauhalf(plus_curve, plus_stage, args.N, "curve")

            # stage -eps
            minus_stage = stage_root / f"minus_topo={args.topology}_N{args.N}_g{g}_p{p}_eps{-abs(args.eps):.5f}"
            stage_curve_for_tauhalf(minus_curve, minus_stage, args.N, "curve")

            # tauhalf (+)
            plus_tau_dir = tau_root / f"plus" / f"topo={args.topology}" / f"N={args.N}" / f"g={g}" / f"p={p}"
            plus_tau_csv = run_tauhalf(base_stage, plus_stage, plus_tau_dir,
                                       args.tmin, args.tmax, args.grid_dt, args.abs_delta, args.normalize)
            plus_tau = parse_tauhalf_summary(plus_tau_csv)
            plus_lb  = tauhalf_lower_bound(plus_tau, args.tmax)

            # tauhalf (-)
            minus_tau_dir = tau_root / f"minus" / f"topo={args.topology}" / f"N={args.N}" / f"g={g}" / f"p={p}"
            minus_tau_csv = run_tauhalf(base_stage, minus_stage, minus_tau_dir,
                                        args.tmin, args.tmax, args.grid_dt, args.abs_delta, args.normalize)
            minus_tau = parse_tauhalf_summary(minus_tau_csv)
            minus_lb  = tauhalf_lower_bound(minus_tau, args.tmax)

            # symmetry scores (use tau_half_lb for censored safety)
            tpeak_sym = effect_symmetry(plus_tau.get("t_peak", float("nan")), minus_tau.get("t_peak", float("nan")))
            auc_sym   = effect_symmetry(plus_tau.get("auc_delta", float("nan")), minus_tau.get("auc_delta", float("nan")))
            tau_sym   = effect_symmetry(plus_lb, minus_lb)
